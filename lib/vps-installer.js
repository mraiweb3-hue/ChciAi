...
function createSSHCommand(remoteCommand) {
  if (process.env.SSH_PRIVATE_KEY) {
    // Encode key to base64 to avoid shell escaping issues
    const sshKeyBase64 = Buffer.from(process.env.SSH_PRIVATE_KEY).toString('base64')
    return `(umask 077; echo '${sshKeyBase64}' | base64 -d > /tmp/ssh_key_$$ && ssh -o StrictHostKeyChecking=no -i /tmp/ssh_key_$$ ${VPS_USER}@${VPS_HOST} '${remoteCommand}'; rm -f /tmp/ssh_key_$$)`
  } else {
