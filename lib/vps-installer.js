...
export async function installOpenClawOnVPS(clientId, port, email) {
  try {
    ...
    if (process.env.SSH_PRIVATE_KEY) {
      ...
      const keyPath = path.join(tmpDir, `ssh_key_${Date.now()}`)
      
      await fs.promises.writeFile(keyPath, process.env.SSH_PRIVATE_KEY, { mode: 0o600 })
      
      command = `ssh -o StrictHostKeyChecking=no -i ${keyPath} ${VPS_USER}@${VPS_HOST} 'bash ${INSTALL_SCRIPT} ${sanitizedClientId} ${port} ${sanitizedEmail}'`
      
      // Cleanup key after use
      setTimeout(() => {
        fs.promises.unlink(keyPath).catch(() => {})
      }, 60000) // Delete after 1 minute
    } else {
      // Fallback without key (will fail if password auth not enabled)
