...
export async function installOpenClawOnVPS(clientId, port, email) {
  try {
    ...
    // Execute installation script via SSH with key from stdin
    let command
    
    if (process.env.SSH_PRIVATE_KEY) {
      // Use process substitution to pass key via stdin (more secure, no temp file)
      const sshKey = process.env.SSH_PRIVATE_KEY
      
      // Create command that uses the key without writing to disk
      command = `(umask 077; echo '${sshKey}' > /tmp/ssh_key_$$ && ssh -o StrictHostKeyChecking=no -i /tmp/ssh_key_$$ ${VPS_USER}@${VPS_HOST} 'bash ${INSTALL_SCRIPT} ${sanitizedClientId} ${port} ${sanitizedEmail}'; rm -f /tmp/ssh_key_$$)`
    } else {
      // Fallback without key
      command = `ssh -o StrictHostKeyChecking=no ${VPS_USER}@${VPS_HOST} 'bash ${INSTALL_SCRIPT} ${sanitizedClientId} ${port} ${sanitizedEmail}'`
    }
